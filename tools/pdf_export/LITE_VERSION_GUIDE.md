...
### 1. 简单模式（保持顺序 + topic 分组）

仅列出词条，词条按文件顺序排列，导出时仍按 topic 分组，但组内保持白名单顺序：
...
- **简单模式**：词条仍按原有 topic 字段分组，但保持白名单顺序
- **自定义分组模式**：使用 `##` 标题完全覆盖原有分组，按你指定的结构组织

**提示**：

- 分组标题（`##`）会作为 PDF 目录的章节标题
- 词条严格按照在文件中的顺序排列
- 可以根据需要调整词条和分组的顺序
