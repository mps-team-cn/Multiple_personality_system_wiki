...
echo   3. python tools/pdf_export/export_to_pdf.py --pdf-engine=tectonic --cjk-font="Microsoft YaHei"
echo   4. python tools/fix_md.py
echo   5. markdownlint "**/*.md" --ignore "node_modules" --ignore "tools/pdf_export/vendor"
echo.
echo 可选参数:
...
REM 4. 自动修复 Markdown
if %SKIP_FIX_MD%==0 (
    echo [4/5] 自动修复 Markdown 格式...
    python tools/fix_md.py
    if errorlevel 1 echo 警告: Markdown 修复失败
    echo.
) else (
    echo [4/5] 已跳过: Markdown 自动修复
    echo.
)

REM 5. 运行 markdownlint
if %SKIP_MARKDOWNLINT%==0 (
    echo [5/5] 运行 markdownlint 校验...
    markdownlint "**/*.md" --ignore "node_modules" --ignore "tools/pdf_export/vendor"
    if errorlevel 1 echo 警告: markdownlint 校验发现问题
    echo.
) else (
    echo [5/5] 已跳过: markdownlint 校验
    echo.
)
