...
echo   -h, --help, /?         显示本帮助信息
echo.
echo 注意: MkDocs Material 使用内置搜索,不再需要单独生成搜索索引
echo.
goto end
...
REM 注意: MkDocs Material 使用内置搜索功能,不再需要 build_search_index.py
REM 该工具仅用于已弃用的 Docsify 框架

REM 6. 自动修复 Markdown
if %SKIP_FIX_MD%==0 (
    echo [6/7] 自动修复 Markdown 格式...
    python tools/fix_md.py
    if errorlevel 1 echo 警告: Markdown 修复失败
    echo.
) else (
    echo [6/7] 已跳过: Markdown 自动修复
    echo.
)

REM 7. 运行 markdownlint
if %SKIP_MARKDOWNLINT%==0 (
    echo [7/7] 运行 markdownlint 校验...
    markdownlint "**/*.md" --ignore "node_modules" --ignore "tools/pdf_export/vendor"
    if errorlevel 1 echo 警告: markdownlint 校验发现问题
    echo.
) else (
    echo [7/7] 已跳过: markdownlint 校验
    echo.
)
