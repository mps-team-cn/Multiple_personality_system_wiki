...
title: MID-60 多维解离量表说明 - MPSTEAM Wiki
description: 介绍 MID-60 的基本用途、适用边界、与 DES-II 的区别，以及解离相关资料的中文阅读导航。
keywords: MID-60, 多维解离量表, 解离, DID, DES-II
topic: 诊断与临床
updated: 2026-05-19
---
...
!!! info "速览 Clinician's Summary"
    ...
    **计分方法**：受试者对 60 道题目按 0–10 评分（0 = 从不，10 = 总是），**总分 = 全体 60 题均值 × 10（范围 0–100%）**。页面同时展示 12 个相关子量表的结果区间，便于观察症状分布。

    **阅读方式**：本页沿用公开资料中的结果区间与参考阈值，帮助读者理解分数落点；这些信息仅用于筛查与资料整理，**不能单独视为诊断标准**。
!!! warning "重要声明"
...
!!! tip "使用说明"
    ...
    - 请尽量基于 **最近一个月** 的真实体验作答,而不是只回忆极端情况或单次事件。
    - 建议按直觉作答,不要为追求“准确标签”而反复修改答案。
    - 诚实作答比“作答得像某种诊断”更重要,本工具不存储个人数据。
    - 每一道题用滑块选择 0–10:表示该体验出现的频率,0 表示"从不",10 表示"总是"。
    - 建议按直觉作答;尽量不要反复修改,以首次直觉为准。
    - **计分方法**:总分 = 全体 60 题均值 × 10（范围 0–100%）,结果区会按公开资料中的 **21% 参考线** 提示需要进一步留意的分数区间。
...
!!! info "分数解释（筛查用途）"
    ...
    **重要提示**:分数需要结合具体困扰与功能受损情况讨论;**高分≠诊断,低分≠无碍**。页面采用的结果区间用于帮助理解分数分布,不应被当作单独的诊断标准。

!!! note "子量表说明（依据 NovoPsych 公示资料）"

    MID-60 提供与不同症状维度相关的子量表信息,用于帮助整理体验模式,不直接等同于诊断结论:
...
!!! warning "重要说明"
    ...
    - 本页不提供诊断结论。
    - 量表结果需要结合临床访谈、病史、功能损害、风险评估与现实处境综合判断。
    - 任何量表都不能单独替代专业判断。
    - 若当前存在急性自伤、自杀、暴力风险或现实安全风险，应优先联系当地急救、危机热线或线下专业机构，而不是继续依赖量表自行判断；可参考 [危机与支援资源](Crisis-And-Support-Resources.md)。
...
进一步解释时,仍需回到具体病史、现实功能与访谈资料中,而不是只看总分或单个子量表高低。

## 常见误区

- **MID-60 分数高不等于一定是 DID**：高分只能提示“值得进一步了解或评估”,不能直接推出诊断名称。
- **分数低也不等于完全没有解离体验**：有些人只在特定情境下出现明显体验,也有人会低估或难以回忆自己的状态。
- **量表结果不能脱离访谈和现实功能情况解释**：是否影响学习、工作、关系与安全,往往比单个数字更重要。
- **不建议反复刷量表来确认身份或状态**：频繁重复作答容易放大焦虑、强化自我监测偏差,也会让结果更难解释。
- **作答情境会影响结果**：睡眠不足、物质影响、急性应激或情绪危机期间,都可能让结果失真。
