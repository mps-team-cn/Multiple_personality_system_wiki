...
1. **思维并行** ：各成员在脑内同步进行不同的推理、讨论或记忆检索，常被形容为“脑内多线程对话”或“一边行动一边内部会议”。
1. **执行并行** ：较少见的形态，指不同成员分别主导身体的不同部分或任务，如一位控制书写，另一位负责口头回应，通常仅在高度训练或特殊状况下短暂出现。
...
1. **记录并行日志** ：使用内部记录或共享文档追踪任务、情绪与成效，帮助分析可行的并行模式。
1. **循序训练** ：由短时、低复杂度的并行练习开始，逐步延长时间或提升任务难度，降低解离失控风险。
1. **搭配地面化技巧** ：结合呼吸、身体扫描等接地练习维持身体定向，避免因注意力分裂而失去现实感。
