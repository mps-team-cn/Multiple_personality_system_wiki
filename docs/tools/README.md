...
| 脚本/模块                                                        | 功能摘要                                                              | 常用用法                                                                                                |
| ------------------------------------------------------------ | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `tools/build_partitions_cn.py`                               | 自动生成主题分区索引页，根据词条的 topic 字段聚合分类，构建时自动运行                       | MkDocs 构建时自动执行；CI 自动修复（auto-fix-entries）也会运行；亦可手动：`python3 tools/build_partitions_cn.py` |
...
- ⚠️ 排除列表在 **目录扫描和单文件检查** 时均生效（v3.12.1+ 修复）
- ⚠️ 更新排除列表后建议运行测试确保所有文件存在
- 💡 建议在提交前本地运行此工具，避免 CI 失败

**v3.12.1 更新 (2025-10-14):**
