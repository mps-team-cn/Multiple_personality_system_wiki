...
使用方法:
    python tools/cli.py lint      # 运行所有检查
    python tools/cli.py fix       # 修复所有问题
    python tools/cli.py build     # 构建站点
    python tools/cli.py serve     # 启动开发服务器
"""
...
@click.group()
def cli():
    """Multiple Personality System Wiki 维护工具"""
    pass
...
def lint(verbose):
    """运行所有质量检查"""
    click.echo("🔍 运行 Markdown 格式检查...")
    # 调用 fix_markdown.py --dry-run

    click.echo("🔗 检查链接有效性...")
    # 调用 check_links.py

    click.echo("✅ 验证词条结构...")
    # 调用 gen-validation-report.py

@cli.command()
@click.option('--verbose', '-v', is_flag=True)
def fix(verbose):
    """自动修复所有问题"""
    click.echo("🔧 修复 Markdown 格式...")
    # 调用 fix_markdown.py
...
def serve(port):
    """启动开发服务器"""
    import subprocess
    subprocess.run(['mkdocs', 'serve', '-a', f'127.0.0.1:{port}'])
...
python tools/cli.py lint

# 修复问题

python tools/cli.py fix

# 启动开发服务器

python tools/cli.py serve --port 8080
```
...
lint:  ## 运行所有检查
	@echo "🔍 检查 Markdown 格式..."
	python3 tools/fix_markdown.py docs/ --dry-run
	@echo "🔗 检查链接有效性..."
	python3 tools/check_links.py --root .

fix:  ## 自动修复问题
	@echo "🔧 修复 Markdown 格式..."
	python3 tools/fix_markdown.py docs/ --verbose
...
| 任务 | 预计时间 | 负责人 | 状态 |
|------|---------|--------|------|
| 添加 GitHub Actions | 2-3 小时 | - | ⏳ 待开始 |
| 创建 pre-commit hooks | 1-2 小时 | - | ⏳ 待开始 |
| 锁定依赖版本 | 30 分钟 | - | ⏳ 待开始 |
...
|------|---------|--------|------|
| 改进链接检查工具 | 2-3 小时 | - | ⏳ 待开始 |
| 创建词条完整性验证器 | 3-4 小时 | - | ⏳ 待开始 |
| 统一工具 CLI 入口 | 2-3 小时 | - | ⏳ 待开始 |
| 创建 Makefile | 1 小时 | - | ⏳ 待开始 |
