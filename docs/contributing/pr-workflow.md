...
uv run python3 tools/fix_markdown.py --help
```
...
uv run python3 tools/fix_markdown.py docs/entries/某词条.md
```
...
- [ ] 内部链接与标签正确
- [ ] 本地构建通过（`uv run mkdocs build --strict`）
- [ ] Lint 检查通过
- [ ] 已更新最后修改时间
...
**解决** ：运行 `uv run python3 tools/fix_markdown.py` 自动修复
...
- 检查是否使用相对路径
- 确认目标文件存在
- 使用 `uv run mkdocs serve` 本地验证

### 6.3 构建问题

**问题** ：`uv run mkdocs build --strict` 失败
...
uv run python3 tools/gen-validation-report.py
