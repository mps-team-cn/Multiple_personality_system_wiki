...
参考现有词条，选择合适的标签，具体内容参考[tagging-standard](tagging-standard.md)
...
!!! tip "目的"
    保持全站 Markdown 格式统一，确保 MkDocs Material 渲染一致性与可维护性。

### 12.1 基本要求

- 使用 **UTF-8 编码**，文件结尾保留 **单一换行符**
- 每个段落之间 **空一行**，避免粘连渲染
- 标题层级从 `#`（一级标题）开始，依次递进，不跳级
- 代码块使用三反引号 <code>```</code> 包围，并注明语言（如 `bash`, `python`, `markdown`）
- 提示块（Admonition）需四空格缩进，参照 `AGENTS.md §7.2`
...
- `-` 可在 MkDocs Material 中保持一致缩进与主题符号
- `*` 和 `+` 可能触发渲染不一致或影响插件解析

### 12.3 链接与图片格式

- 内部链接使用相对路径（`[DID](DID.md)`）
- 外部链接使用完整 URL，并在必要时注明来源
- 图片引用路径一律使用 `../assets/` 开头，详见 §4 图片资源组织
- Markdown 链接与图片路径中不得出现空格或中文标点

### 12.4 文本与符号约定

- 中英文之间保留一个空格（例如：“解离（Dissociation）是一种…”）
- 使用全角标点（：，。；“”）而非半角
- 引号统一使用中文全角书名号与引号：「」或“”
- 术语首现时加粗或定义，如：**系统成员（system member）**
- 表格需加表头行，不得留空单元格
- 不得混用 HTML 标签或嵌入样式
