...
??? tip "阅读前说明"
    - 为便于表述，站内的 `MPS` 默认指 `Multiple Personality System`。社群中也常用更中立的 `Plural System`；从理论角度，`MPS` 亦可理解为 `Multiple Consciousness System`（MCS），强调意识多元性。不同译名出现分歧时，以`英文原文`和`上下文`为准。
    - **MPSTEAM** 允许使用`AI`辅助翻译、整理和润色文本，但所有`AI`生成内容均作为草稿处理。正式发布前必须经过 **人工审核** ；涉及`专业`、`伦理`、`创伤`、`多重自我`相关内容时，应进行 **交叉审核** 。AI 可以参与工作流程，但不承担内容责任。
    - 对于部分外文资料中的术语或表述，若无法与中文语境形成完全一一对应,`Wiki`编辑组会在尊重原意的基础上，采用更符合中文阅读习惯的同义表达，并尽量保留清晰、可追溯的参考来源。
    - 由于本`Wiki`创立时间不是很长，难免有一些疏漏或问题，希望大家谅解给我们慢慢改进的时间。同时`MPS Team`欢迎任何合理且友善的批评建议与指导,对于缺乏事实依据的指控、恶意曲解、情绪化攻击或涉嫌人身攻击的内容，编辑组将不予采纳。
...
查看完整 [更新日志](changelog.md) | [下载 PDF 版本](https://github.com/mps-team-cn/Multiple_personality_system_wiki/releases/download/v4.3.0/Multiple_Personality_System_wiki.pdf)
